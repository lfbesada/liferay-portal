...
public class EditFragmentEntryMVCActionCommand
	...
	protected void doProcessAction(
		...
		if (draftFragmentEntry.getType() == FragmentConstants.TYPE_INPUT) {
			String fileTypes = ParamUtil.getString(actionRequest, "fieldTypes");

			JSONArray fileTypesJSONArray = JSONFactoryUtil.createJSONArray();

			if (Validator.isNotNull(fileTypes)) {
				fileTypesJSONArray = JSONFactoryUtil.createJSONArray(
					fileTypes.split(StringPool.COMMA));
			}

			JSONObject propertiesJSONObject = JSONFactoryUtil.createJSONObject(
				draftFragmentEntry.getProperties());

			propertiesJSONObject.put("fieldTypes", fileTypesJSONArray);

			draftFragmentEntry.setProperties(propertiesJSONObject.toString());
		}
