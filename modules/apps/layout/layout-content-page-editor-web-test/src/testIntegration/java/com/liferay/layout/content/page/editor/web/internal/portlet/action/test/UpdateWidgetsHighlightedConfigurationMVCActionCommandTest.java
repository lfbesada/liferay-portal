...
public class UpdateWidgetsHighlightedConfigurationMVCActionCommandTest {
	...
	@Test
	public void testUpdateWidgetsHighlightedConfiguration() throws Exception {
		String[] defaultHighlightedPortletIds =
			_getDefaultHighlightedPortletIds();

		_assertUpdateHighlightedWidget(
			ArrayUtil.append(
		...
		_assertUpdateHighlightedWidget(
			...
			defaultHighlightedPortletIds[0], false, TestPropsValues.getUser());
	}

	@Test
	public void testUpdateWidgetsHighlightedConfigurationAssertOwnerByUser()
		throws Exception {
	...
	private void _assertUpdateHighlightedWidget(
		...
		JSONObject jsonObject = ReflectionTestUtil.invoke(
			_mvcActionCommand, "_updateWidgetsHighlightedConfiguration",
			new Class<?>[] {ActionRequest.class},
			mockLiferayPortletActionRequest);

		Assert.assertTrue(jsonObject.has("highlightedWidgets"));
		Assert.assertFalse(jsonObject.has("error"));

		JSONArray highlightedWidgetsJSONArray = jsonObject.getJSONArray(
			"highlightedWidgets");

		Assert.assertEquals(
			expectedHighlightedPortletIds.length,
			highlightedWidgetsJSONArray.length());

		for (int i = 0; i < highlightedWidgetsJSONArray.length(); i++) {
			JSONObject widgetJSONObject =
				highlightedWidgetsJSONArray.getJSONObject(i);

			Assert.assertTrue(
				ArrayUtil.contains(
					expectedHighlightedPortletIds,
					widgetJSONObject.getString("portletId")));
		}
	}
	...
	@Inject(
		filter = "mvc.command.name=/layout_content_page_editor/update_widgets_highlighted_configuration"
	)
