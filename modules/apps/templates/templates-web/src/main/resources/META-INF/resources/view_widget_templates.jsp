...
<liferay-util:include page="/widget_template_management_bar.jsp" servletContext="<%= application %>" />
...
<c:if test="<%= showHeader %>">
	<c:choose>
		<c:when test="<%= ddmDisplay.isShowBackURLInTitleBar() %>">
			...
			renderResponse.setTitle(ddmDisplay.getViewTemplatesTitle(structure, controlPanel, templatesDisplayContext.isSearch(), locale));
			%>
		...
		<c:otherwise>
			<liferay-ui:header
				...
				title="<%= ddmDisplay.getViewTemplatesTitle(structure, controlPanel, templatesDisplayContext.isSearch(), locale) %>"
			/>
		</c:otherwise>
	</c:choose>
</c:if>
...
<aui:form action="<%= viewTemplateURL.toString() %>" method="post" name="fm">
	<aui:input name="redirect" type="hidden" value="<%= templatesDisplayContext.getTemplateSearchActionURL() %>" />
	...
	>
		<liferay-ui:search-container
			id="<%= templatesDisplayContext.getTemplateSearchContainerId() %>"
			rowChecker="<%= new DDMTemplateRowChecker(renderResponse) %>"
			searchContainer="<%= templatesDisplayContext.getTemplateSearch() %>"
		>
