...
DDMTemplate template = (DDMTemplate)request.getAttribute(TemplatesPortletKeys.DDM_TEMPLATE);
...
if ((structure == null) && (template != null)) {
	structure = templatesDisplayContext.fetchStructure(template);
}
...
if (Validator.isNull(script) && type.equals(DDMTemplateConstants.TEMPLATE_TYPE_DISPLAY)) {
	...
	if (templateHandler != null) {
		...
		script = StringUtil.read(TemplatesWebConfigurationUtil.class.getClassLoader(), TemplatesWebConfigurationUtil.get(TemplatesWebConfigurationKeys.TEMPLATES_TEMPLATE_LANGUAGE_CONTENT, new Filter(language)));
	}
}
...
<portlet:actionURL name="/templates/add_template" var="addTemplateURL">
	<portlet:param name="mvcPath" value="/edit_template.jsp" />
	<portlet:param name="<%= TemplatesPortletKeys.CURRENT_TAB_ID_PARAMETER_NAME %>" value="<%= templatesDisplayContext.getSelectedTab() %>" />
</portlet:actionURL>

<portlet:actionURL name="/templates/update_template" var="updateTemplateURL">
	<portlet:param name="mvcPath" value="/edit_template.jsp" />
	<portlet:param name="<%= TemplatesPortletKeys.CURRENT_TAB_ID_PARAMETER_NAME %>" value="<%= templatesDisplayContext.getSelectedTab() %>" />
</portlet:actionURL>
	...
	<aui:form action="<%= (template == null) ? addTemplateURL : updateTemplateURL %>" cssClass="container-fluid container-fluid-max-xl" enctype="multipart/form-data" method="post" name="fm" onSubmit='<%= "event.preventDefault();" %>'>
		...
		<div class="lfr-form-content">
			...
			<liferay-ui:error exception="<%= TemplateSmallImageNameException.class %>">
				...
				<%
				String[] imageExtensions = templatesDisplayContext.smallImageExtensions();
				%>
			...
			<liferay-ui:error exception="<%= TemplateSmallImageSizeException.class %>">
				...
				<%
				long imageMaxSize = templatesDisplayContext.smallImageMaxSize();
				%>
				...
				<c:choose>
					...
					<c:otherwise>
						<liferay-ui:header
							...
							localizeTitle="<%= false %>"
							showBackURL='<%= ParamUtil.getBoolean(request, "showBackURL", true) %>'
							title="<%= title %>"
						/>
					</c:otherwise>
				</c:choose>
				...
				<aui:fieldset>
					...
					>
						...
						>
							...
							<c:if test="<%= !templatesDisplayContext.autogenerateTemplateKey() %>">
