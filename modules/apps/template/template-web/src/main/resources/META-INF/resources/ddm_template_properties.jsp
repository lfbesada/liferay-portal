...
<div class="form-group-sm template-properties">
	<aui:select changesContext="<%= true %>" cssClass="form-control-sm" helpMessage='<%= (ddmTemplate == null) ? StringPool.BLANK : "changing-the-language-does-not-automatically-translate-the-existing-template-script" %>' label="language" name="language">
		...
		%>
			...
			<aui:option label="<%= sb.toString() %>" selected="<%= Objects.equals(templateDisplayContext.getLanguage(), curLangType) %>" value="<%= curLangType %>" />
	...
	<p class="small">
		<%= templateDisplayContext.getTemplateTypeLocalizedLabel(templateDisplayContext.getClassNameId()) %>
	</p>
	...
	<c:if test="<%= (ddmTemplate == null) && !templateDisplayContext.autogenerateTemplateKey() %>">
	...
	<div class="<%= Objects.equals(smallImageSource, "url") ? "" : "hide" %>" id="<portlet:namespace />smallImageURLContainer">
		...
		<c:if test="<%= templateDisplayContext.isSmallImage() && (ddmTemplate != null) && Validator.isNotNull(ddmTemplate.getSmallImageURL()) %>">
	...
	<div class="<%= Objects.equals(smallImageSource, "file") ? "" : "hide" %>" id="<portlet:namespace />smallImageFileContainer">
		...
		<c:if test="<%= templateDisplayContext.isSmallImage() && (ddmTemplate != null) && (ddmTemplate.getSmallImageId() > 0) %>">
			<p>
