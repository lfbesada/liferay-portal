/**
 * SPDX-FileCopyrightText: (c) 2025 Liferay, Inc. https://liferay.com
 * SPDX-License-Identifier: LGPL-2.1-or-later OR LicenseRef-Liferay-DXP-EULA-2.0.0-2023-06
 */

package com.liferay.headless.admin.site.dto.v1_0;

import com.fasterxml.jackson.annotation.JsonFilter;
import com.fasterxml.jackson.annotation.JsonIgnore;
import com.fasterxml.jackson.annotation.JsonProperty;

import com.liferay.petra.function.UnsafeSupplier;
import com.liferay.petra.string.StringBundler;
import com.liferay.portal.kernel.util.StringUtil;
import com.liferay.portal.vulcan.graphql.annotation.GraphQLField;
import com.liferay.portal.vulcan.graphql.annotation.GraphQLName;
import com.liferay.portal.vulcan.util.ObjectMapperUtil;

import jakarta.annotation.Generated;

import jakarta.validation.Valid;

import jakarta.xml.bind.annotation.XmlRootElement;

import java.io.Serializable;

import java.util.Iterator;
import java.util.Map;
import java.util.Objects;
import java.util.Set;
import java.util.function.Supplier;

/**
 * @author Rubén Pulido
 * @generated
 */
@Generated("")
@GraphQLName(
	description = "A value of a field of type navigation menu.",
	value = "NavigationMenuValue"
)
@JsonFilter("Liferay.Vulcan")
@XmlRootElement(name = "NavigationMenuValue")
public class NavigationMenuValue implements Serializable {

	public static NavigationMenuValue toDTO(String json) {
		return ObjectMapperUtil.readValue(NavigationMenuValue.class, json);
	}

	public static NavigationMenuValue unsafeToDTO(String json) {
		return ObjectMapperUtil.unsafeReadValue(
			NavigationMenuValue.class, json);
	}

	@io.swagger.v3.oas.annotations.media.Schema
	@Valid
	public ItemExternalReference getNavigationMenu() {
		if (_navigationMenuSupplier != null) {
			navigationMenu = _navigationMenuSupplier.get();

			_navigationMenuSupplier = null;
		}

		return navigationMenu;
	}

	public void setNavigationMenu(ItemExternalReference navigationMenu) {
		this.navigationMenu = navigationMenu;

		_navigationMenuSupplier = null;
	}

	@JsonIgnore
	public void setNavigationMenu(
		UnsafeSupplier<ItemExternalReference, Exception>
			navigationMenuUnsafeSupplier) {

		_navigationMenuSupplier = () -> {
			try {
				return navigationMenuUnsafeSupplier.get();
			}
			catch (RuntimeException runtimeException) {
				throw runtimeException;
			}
			catch (Exception exception) {
				throw new RuntimeException(exception);
			}
		};
	}

	@GraphQLField
	@JsonProperty(access = JsonProperty.Access.READ_WRITE)
	protected ItemExternalReference navigationMenu;

	@JsonIgnore
	private Supplier<ItemExternalReference> _navigationMenuSupplier;

	@io.swagger.v3.oas.annotations.media.Schema
	public String getParentItemExternalReferenceCode() {
		if (_parentItemExternalReferenceCodeSupplier != null) {
			parentItemExternalReferenceCode =
				_parentItemExternalReferenceCodeSupplier.get();

			_parentItemExternalReferenceCodeSupplier = null;
		}

		return parentItemExternalReferenceCode;
	}

	public void setParentItemExternalReferenceCode(
		String parentItemExternalReferenceCode) {

		this.parentItemExternalReferenceCode = parentItemExternalReferenceCode;

		_parentItemExternalReferenceCodeSupplier = null;
	}

	@JsonIgnore
	public void setParentItemExternalReferenceCode(
		UnsafeSupplier<String, Exception>
			parentItemExternalReferenceCodeUnsafeSupplier) {

		_parentItemExternalReferenceCodeSupplier = () -> {
			try {
				return parentItemExternalReferenceCodeUnsafeSupplier.get();
			}
			catch (RuntimeException runtimeException) {
				throw runtimeException;
			}
			catch (Exception exception) {
				throw new RuntimeException(exception);
			}
		};
	}

	@GraphQLField
	@JsonProperty(access = JsonProperty.Access.READ_WRITE)
	protected String parentItemExternalReferenceCode;

	@JsonIgnore
	private Supplier<String> _parentItemExternalReferenceCodeSupplier;

	@io.swagger.v3.oas.annotations.media.Schema
	public Boolean getPrivatePages() {
		if (_privatePagesSupplier != null) {
			privatePages = _privatePagesSupplier.get();

			_privatePagesSupplier = null;
		}

		return privatePages;
	}

	public void setPrivatePages(Boolean privatePages) {
		this.privatePages = privatePages;

		_privatePagesSupplier = null;
	}

	@JsonIgnore
	public void setPrivatePages(
		UnsafeSupplier<Boolean, Exception> privatePagesUnsafeSupplier) {

		_privatePagesSupplier = () -> {
			try {
				return privatePagesUnsafeSupplier.get();
			}
			catch (RuntimeException runtimeException) {
				throw runtimeException;
			}
			catch (Exception exception) {
				throw new RuntimeException(exception);
			}
		};
	}

	@GraphQLField
	@JsonProperty(access = JsonProperty.Access.READ_WRITE)
	protected Boolean privatePages;

	@JsonIgnore
	private Supplier<Boolean> _privatePagesSupplier;

	@Override
	public boolean equals(Object object) {
		if (this == object) {
			return true;
		}

		if (!(object instanceof NavigationMenuValue)) {
			return false;
		}

		NavigationMenuValue navigationMenuValue = (NavigationMenuValue)object;

		return Objects.equals(toString(), navigationMenuValue.toString());
	}

	@Override
	public int hashCode() {
		String string = toString();

		return string.hashCode();
	}

	public String toString() {
		StringBundler sb = new StringBundler();

		sb.append("{");

		ItemExternalReference navigationMenu = getNavigationMenu();

		if (navigationMenu != null) {
			if (sb.length() > 1) {
				sb.append(", ");
			}

			sb.append("\"navigationMenu\": ");

			sb.append(String.valueOf(navigationMenu));
		}

		String parentItemExternalReferenceCode =
			getParentItemExternalReferenceCode();

		if (parentItemExternalReferenceCode != null) {
			if (sb.length() > 1) {
				sb.append(", ");
			}

			sb.append("\"parentItemExternalReferenceCode\": ");

			sb.append("\"");

			sb.append(_escape(parentItemExternalReferenceCode));

			sb.append("\"");
		}

		Boolean privatePages = getPrivatePages();

		if (privatePages != null) {
			if (sb.length() > 1) {
				sb.append(", ");
			}

			sb.append("\"privatePages\": ");

			sb.append(privatePages);
		}

		sb.append("}");

		return sb.toString();
	}

	@io.swagger.v3.oas.annotations.media.Schema(
		accessMode = io.swagger.v3.oas.annotations.media.Schema.AccessMode.READ_ONLY,
		defaultValue = "com.liferay.headless.admin.site.dto.v1_0.NavigationMenuValue",
		name = "x-class-name"
	)
	public String xClassName;

	private static String _escape(Object object) {
		return StringUtil.replace(
			String.valueOf(object), _JSON_ESCAPE_STRINGS[0],
			_JSON_ESCAPE_STRINGS[1]);
	}

	private static boolean _isArray(Object value) {
		if (value == null) {
			return false;
		}

		Class<?> clazz = value.getClass();

		return clazz.isArray();
	}

	private static String _toJSON(Map<String, ?> map) {
		StringBuilder sb = new StringBuilder("{");

		@SuppressWarnings("unchecked")
		Set set = map.entrySet();

		@SuppressWarnings("unchecked")
		Iterator<Map.Entry<String, ?>> iterator = set.iterator();

		while (iterator.hasNext()) {
			Map.Entry<String, ?> entry = iterator.next();

			sb.append("\"");
			sb.append(_escape(entry.getKey()));
			sb.append("\": ");

			Object value = entry.getValue();

			if (_isArray(value)) {
				sb.append("[");

				Object[] valueArray = (Object[])value;

				for (int i = 0; i < valueArray.length; i++) {
					if (valueArray[i] instanceof Map) {
						sb.append(_toJSON((Map<String, ?>)valueArray[i]));
					}
					else if (valueArray[i] instanceof String) {
						sb.append("\"");
						sb.append(valueArray[i]);
						sb.append("\"");
					}
					else {
						sb.append(valueArray[i]);
					}

					if ((i + 1) < valueArray.length) {
						sb.append(", ");
					}
				}

				sb.append("]");
			}
			else if (value instanceof Map) {
				sb.append(_toJSON((Map<String, ?>)value));
			}
			else if (value instanceof String) {
				sb.append("\"");
				sb.append(_escape(value));
				sb.append("\"");
			}
			else {
				sb.append(value);
			}

			if (iterator.hasNext()) {
				sb.append(", ");
			}
		}

		sb.append("}");

		return sb.toString();
	}

	private static final String[][] _JSON_ESCAPE_STRINGS = {
		{"\\", "\"", "\b", "\f", "\n", "\r", "\t"},
		{"\\\\", "\\\"", "\\b", "\\f", "\\n", "\\r", "\\t"}
	};

	private Map<String, Serializable> _extendedProperties;

}