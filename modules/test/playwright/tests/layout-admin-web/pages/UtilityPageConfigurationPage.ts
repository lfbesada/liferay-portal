...
export class UtilityPageConfigurationPage {
	...
	constructor(page: Page) {
		...
		this.htmlDescriptionPlaceholder = page.getByPlaceholder(
			'Description'
		);
		this.htmlTitlePlaceholder = page.getByPlaceholder(
			'Title'
		);
	...
	async goTo(siteFriendlyUrlPath: string, title: string) {
		await this.utilityPagesPage.goTo(siteFriendlyUrlPath);
		await this.utilityPagesPage.clickOnAction("Configure", title);
	}

	async setUtilityPageConfiguration(htmlDescription: string, htmlTitle: string, siteFriendlyUrlPath: string, title: string) {
		await this.goTo(siteFriendlyUrlPath, title);
